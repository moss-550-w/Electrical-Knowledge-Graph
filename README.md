...
- **原理动图演示** — 三条金线主干顶点（旋转磁场 / 储能双向功率流 / 磁耦合传能）内嵌自制 Lottie 矢量动画，自动循环播放、可暂停；lottie-web 按需懒加载、离开浮层即销毁实例
...
| 原理动图 | lottie-web 5.x（自制 SVG 矢量动画，按需懒加载、独立 chunk） |
| 状态管理 | Pinia 2 |
...
│   └── lottie/              # 自制原理动图 Lottie 素材（旋转磁场/功率流/磁耦合）
├── styles/
│   └── dark.css             # 深色星空主题样式
...
- [x] 深色主题与新手引导
- [x] 节点自制原理动图（Lottie）接入（旋转磁场 / 储能功率流 / 磁耦合传能）
- [ ] 用户自定义路径保存与分享
- [ ] 移动端手势优化
- [ ] 离线 PWA 支持
