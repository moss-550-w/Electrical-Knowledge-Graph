...
- **原理动图演示** — 52/60 个金线节点（覆盖率 86.7%）及 4 个关键骨架节点（Boost 升压 / 并网逆变器 / 电力系统结构 / 变压器）内嵌自制 Lottie 矢量动画，涵盖旋转磁场、SVPWM 六扇区、dq 坐标变换、开关波形、PWM 调制、三相正弦、傅里叶谐波叠加、闭环阶跃响应、卡尔曼估计、CC-CV 充电、磁耦合传能、电磁波传播、MPPT 爬山、锁相同步、光伏 I-V 特性、Boost 升压、孤岛跳闸等 29 类原型；自动循环播放、可暂停，lottie-web 按需懒加载、离开浮层即销毁实例
...
- **3D 知识依赖塔复盘** — Three.js 场景将学习路径升维为信息可视化：**Y 轴=依赖深度**（数学/电磁底座沉底、工程应用升顶）、**XZ 方位=学科归属**（同学科同方位），路径化作穿过学科分层的发光螺旋脊柱（TubeGeometry）；融合路径节点的直接邻居作暗色「关联节点」环绕浮现，体现"路径是大图中的一条线"；支持**沿路径漫游**（相机依次飞历各节点、上一步/下一步/暂停）、点击节点平滑聚焦、悬停高亮，分层地面标注「第 N 层 · 基础/核心/应用」
...
| 类型 | 数量 | 说明 |
|---|---|---|
| 金线节点（L3·完备） | 60 | 含完整公式推导、逐步推导步骤、工程场景描述、跨学科关联 |
| 骨架节点（L1/L2） | 45 | 涵盖 8 个学科方向，全部配有独立详情文件（概述 + 核心公式 + 应用场景），其中 34 个已升级至 L2 |
| 金线主干 | 4 | 电机控制 ⚙️ · 储能 BMS 🔋 · 无线充电 🔌 · 光伏并网 ☀️ |
...
**金线主干四 · 光伏并网**：光伏并网发电系统 → 光伏阵列建模 / MPPT 最大功率跟踪 / 并网逆变器控制 / 锁相环同步 / 孤岛检测 → 三相逆变 / Boost 升压 / dq 坐标变换 / 并网逆变器 → 半导体物理 / 电力电子基础 / 控制理论

> 四条主干共享电力电子、电磁学与数学底座，在图谱中自然交汇，体现知识的网状关联。
...
| 原理动图 | lottie-web 5.x（29 类自制 SVG 矢量动画原型，按需懒加载、独立 chunk） |
| 状态管理 | Pinia 2 |
...
│   ├── skeletonGraph.json   # 45 个 L1/L2 骨架节点（其中 34 个 L2）
...
- [x] 「光伏并网发电」金线主干（PV 阵列 → MPPT → 并网逆变 → 锁相同步 → 孤岛检测，6 个 L3 节点 + 四主线切换）
- [x] 深色主题与新手引导
- [x] 节点自制原理动图（Lottie）接入（29 类原型，覆盖 86.7% 金线节点 + 4 个关键骨架节点）
